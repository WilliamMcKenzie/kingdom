...
func (character *Character) Apply() {
	...
	character.health = min(character.health, health)
...
// Only the engine loop sends or closes this stream. A slow connection cannot stall the world.
func (character *Character) Send(data []byte) {
	if character.disconnected {
		return
	}
	select {
	case character.send <- data:
	default:
